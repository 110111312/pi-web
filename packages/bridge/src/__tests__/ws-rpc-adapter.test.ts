...
describe("WsRpcAdapter", () => {
  ...
  describe("discovery commands", () => {
    ...
    it("should handle write_workspace_file command", async () => {
      ...
      const filePath = path.join(tmpDir, "src", "notes.md");
      fs.mkdirSync(path.dirname(filePath), { recursive: true });
      ...
      expect(response.payload.success).toBe(true);
      expect(response.payload.data.path).toBe("src/notes.md");
      expect(response.payload.data.absolutePath).toBe(
    ...
    it("should reject write_workspace_file outside the workspace", async () => {
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "pi-web-write-guard-test-"),
      );
      const outsideDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "pi-web-write-outside-test-"),
      );
      const outsideFile = path.join(outsideDir, "outside.txt");
    ...
    it("should reject write_workspace_file with binary content", async () => {
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "pi-web-write-binary-test-"),
      );
      const filePath = path.join(tmpDir, "c.bin");
      ...
      const command: RpcCommand = {
        ...
        type: "write_workspace_file",
        path: "c.bin",
        // String with embedded NUL byte — null check should reject.
      ...
      expect(response.payload.success).toBe(false);
      expect(response.payload.error).toMatch(/binary/i);
      // Placeholder must remain untouched.
  ...
  describe("listGitRepos", () => {
    function setupRepo(files: Record<string, string> = {}): string {
      ...
      // Always create at least one file so `git commit` has something
      // to commit (empty commits aren't allowed without --allow-empty).
      const allFiles =
    ...
    it("returns just the workspace root when no nested repos exist", async () => {
      const tmpDir = setupRepo({ "README.md": "hi\n" });
      ...
      expect(repos[0]!.label).toBe(path.basename(tmpDir));
    ...
    it("is fast even on large directories (no git commands)", async () => {
      const tmpDir = setupRepo();
      // Create many fake subdirs that aren't git repos
      for (let i = 0; i < 100; i++) {
        fs.mkdirSync(path.join(tmpDir, `pkg-${i}`), { recursive: true });
      }
      ...
      // Should be well under 1 second — no git processes spawned
