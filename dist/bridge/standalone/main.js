...
import { loadStandaloneRuntime } from "./runtime.js";
import { existsSync, realpathSync } from "node:fs";
import { dirname, join, resolve } from "node:path";
import { fileURLToPath } from "node:url";
//#region src/standalone/main.ts
...
function findNearestWebDist(startDir) {
...
function resolveDefaultStaticDir(cwd) {
	const candidates = [
		findNearestWebDist(cwd),
		findNearestWebDist(process.cwd()),
		findNearestWebDist(dirname(fileURLToPath(import.meta.url)))
	];
	for (const candidate of candidates) if (candidate) return resolve(candidate);
}
function parseStandaloneMainOptions(argv) {
	...
	for (let index = 0; index < argv.length; index++) {
		...
		switch (token) {
			...
			case "--port": {
				...
				if (!next || next.startsWith("--")) throw new Error("Missing value for --port");
...
async function runStandaloneBridge(runtime, config, options, entryFile, backend, sessionRegistry) {
	let resolveStopped;
	...
	if (options.staticDir) console.log(`[pi-web] Static Dir: ${options.staticDir}`);
...
async function runStandaloneMain() {
	let options;
	try {
	...
	const sessionRegistry = new DetachedSessionRegistry(backend.context.state.cwd);
	try {
		while (true) {
			const runtime = await loadStandaloneRuntime(thisFile);
			if (!await runStandaloneBridge(runtime, {
				...runtime.DEFAULT_BRIDGE_CONFIG,
				port: options.port,
				staticDir: options.staticDir
			}, options, thisFile, backend, sessionRegistry)) return 0;
...
const thisFile = fileURLToPath(import.meta.url);
if (invokedPath && realpathSync(resolve(invokedPath)) === realpathSync(resolve(thisFile))) runStandaloneMain().then((code) => {
...
//#endregion
export { parseStandaloneMainOptions };
